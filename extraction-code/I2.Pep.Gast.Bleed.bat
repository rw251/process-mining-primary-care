...
REM Get GP and NSAID meds for all patients with a PEP/GiB
bcp "select s.PatID, EntryDate, ReadCode, CodeValue, s.CodeUnits from [%SMASH.DB%].[dbo].SIR_ALL_Records_No_Rubric s inner join (select PatID, min(EntryDate) as [date] from [%SMASH.DB%].[dbo].SIR_ALL_Records_Narrow where ReadCode in (%BLEED_CODES%) group by PatID) sub on sub.date < s.EntryDate and s.PatID = sub.PatID inner join [%SMASH.DB%].[dbo].drugCodes d on d.code = s.ReadCode where d.id in ('GAST1','PPI_COD','NSAID_COD') and CodeValue is not null and CodeValue > 0 order by sub.PatID, EntryDate" queryout I2\gast_and_nsaid_bleed_patients.txt -c -T  -b 10000000

REM goto med alg directory to process the data
cd medalgs
node index.js -a ..\I2\gast_and_nsaid_bleed_patients.txt
CALL npm run -s sort ..\I2\gast_and_nsaid_bleed_patients.txt.done > ..\I2\gast_and_nsaid_bleed_patients.txt.done.sorted

perl parse_drug_file.pl ..\I2\gast_and_nsaid_bleed_patients.txt.done.sorted

cd ..

REM now get bleed events
bcp "select s.PatID, EntryDate, 'BLEED' from [%SMASH.DB%].[dbo].SIR_ALL_Records_Narrow s inner join (select PatID, min(EntryDate) as [date] from [%SMASH.DB%].[dbo].SIR_ALL_Records_Narrow where ReadCode in (%BLEED_CODES%) group by PatID) sub on sub.date <= s.EntryDate and s.PatID = sub.PatID where ReadCode in (%BLEED_CODES%) or ReadCode like 'J68%%' group by s.PatID, EntryDate order by s.PatID, EntryDate" queryout I2\gast_and_nsaid_bleed_patients_bleed_events.txt -c -T  -b 10000000

REM now get died events
bcp "SELECT PatID, max(EntryDate) as [date], 'DIED' as [event] FROM [%SMASH.DB%].[dbo].SIR_ALL_Records_Narrow WHERE ReadCode IN (SELECT code FROM [%SMASH.DB%].[dbo].drugCodes WHERE id = 'Death') GROUP BY PatID UNION SELECT p.patid, CAST(year_of_death as varchar) + '-' + CAST(month_of_death as varchar) + '-28', 'DIED' FROM [%SMASH.DB%].[dbo].patients p INNER JOIN ( SELECT patid FROM [%SMASH.DB%].[dbo].patients WHERE dead = 1 EXCEPT SELECT PatID FROM [%SMASH.DB%].[dbo].SIR_ALL_Records_Narrow WHERE ReadCode IN (SELECT code FROM [%SMASH.DB%].[dbo].drugCodes WHERE id = 'Death') 	GROUP BY PatID ) sub on sub.patid = p.patid" queryout I2\gast_and_nsaid_bleed_patients_died_events.txt -c -T  -b 10000000

REM TIDY UP
del I2\gast_and_nsaid_bleed_patients.txt
del I2\gast_and_nsaid_bleed_patients.txt.done
del I2\gast_and_nsaid_bleed_patients.txt.done.sorted

move I2\gast_and_nsaid_bleed_patients.txt.done.sorted.processed I2\med-events.txt
copy I2\gast_and_nsaid_bleed_patients_bleed_events.txt + I2\gast_and_nsaid_bleed_patients_died_events.txt I2\other-events.txt

%ZIP.EXE% a I2.tar I2\med-events.txt I2\other-events.txt
%ZIP.EXE% a -tzip I2.tgz I2.tar
