...
REM Get GP and NSAID meds for all patients over 65
bcp "select s.PatID, EntryDate, ReadCode, CodeValue, s.CodeUnits from [%SMASH.DB%].[dbo].SIR_ALL_Records_No_Rubric s inner join [%SMASH.DB%].[dbo].patients p on p.patid = s.PatID inner join [%SMASH.DB%].[dbo].drugCodes d on d.code = s.ReadCode where d.id in ('GAST1','PPI_COD','NSAID_COD') and CodeValue is not null and CodeValue > 0 and year_of_birth < 1954 and EntryDate > CAST(64+year_of_birth as varchar)+'/07/01' order by s.PatID, EntryDate" queryout I1\age65_bleed_patients.txt -c -T  -b 10000000

REM goto med alg directory to process the data
cd medalgs
node index.js -a ..\I1\age65_bleed_patients.txt
CALL npm run -s sort ..\I1\age65_bleed_patients.txt.done > ..\I1\age65_bleed_patients.txt.done.sorted

perl parse_drug_file.pl ..\I1\age65_bleed_patients.txt.done.sorted

cd ..

REM now get age 65 events
bcp "select patid, CONVERT(datetime,CAST(65+year_of_birth as varchar)+'/01/01') as [date], 'TURNED 65' as [event] from [%SMASH.DB%].[dbo].patients where year_of_birth < 1954" queryout I1\age65_events.txt -c -T  -b 10000000
REM now get bleed events
bcp "select PatID, EntryDate, 'BLEED' from [%SMASH.DB%].[dbo].SIR_ALL_Records_No_Rubric s inner join [%SMASH.DB%].[dbo].patients p on p.patid = s.PatID where (ReadCode in (%BLEED_CODES%) or ReadCode like 'J68%%') and year_of_birth < 1954 and EntryDate > CAST(64+year_of_birth as varchar)+'/07/01' group by PatID, EntryDate order by PatID, EntryDate" queryout I1\bleed_events.txt -c -T  -b 10000000

REM now get died events
bcp "SELECT PatID, max(EntryDate) as [date], 'DIED' as [event] FROM [%SMASH.DB%].[dbo].SIR_ALL_Records_Narrow WHERE ReadCode IN (SELECT code FROM [%SMASH.DB%].[dbo].drugCodes WHERE id = 'Death') GROUP BY PatID UNION SELECT p.patid, CAST(year_of_death as varchar) + '-' + CAST(month_of_death as varchar) + '-28', 'DIED' FROM [%SMASH.DB%].[dbo].patients p INNER JOIN ( SELECT patid FROM [%SMASH.DB%].[dbo].patients WHERE dead = 1 EXCEPT SELECT PatID FROM [%SMASH.DB%].[dbo].SIR_ALL_Records_Narrow WHERE ReadCode IN (SELECT code FROM [%SMASH.DB%].[dbo].drugCodes WHERE id = 'Death') 	GROUP BY PatID ) sub on sub.patid = p.patid" queryout I1\age65_patients_died_events.txt -c -T  -b 10000000

REM TIDY UP

move I1\age65_bleed_patients.txt.done.sorted.processed I1\med-events.txt
copy I1\age65_events.txt + I1\bleed_events.txt + I1\age65_patients_died_events.txt I1\other-events.txt

%ZIP.EXE% a I1.tar I1\med-events.txt I1\other-events.txt
%ZIP.EXE% a -tzip I1.tgz I1.tar
